...
def process_table(table: TableConfig, path: Path, output_path: Path, src: str) -> None:
    # automatic mapping from the code in utils.py
    #print(table.base_extension)
    #print(table.events[0])
    #print(table.events[0].field_names)
    table_paths = {
    ...
    for event in table.events:
        # contains the col names which are maped to the events rows
        print("!event", event.field_names, "event_end")
        field_names = []


        # key = target col name, value = src col name
        extension = {
            key : (event.fields.extension[key] if key in event.fields.extension else value)
            for key, value in table.base_extension.items()
        } 
        print(extension)
        print("!extension", event.fields.extension)
        event.fields.extension = extension
        print("!extension", event.fields.extension)
